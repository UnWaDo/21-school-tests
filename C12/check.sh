...
if test "$2" = "test" ; then
	echo "\nWith 'test' only C files from tests/$f_name/ dir would be compiled"
	check_with_tests_only=1
fi
...
if ! test -d "$GIT_REPO/$f_name" ; then
	...
	elif test $check_with_tests_only -eq "1" ; then
		echo "\nCompiling without general files..."
        gcc -Wall -Wextra -Werror "$GIT_REPO/tests/$f_name/"*.c -o "$GIT_REPO/$f_name.out"
